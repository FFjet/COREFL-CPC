    ...
    if (type == "int") {
      ...
      if (key == "real") {
        ...
        if (num == 2) {
          ...
          printf("The number of values in the range %s is not correct.\n", name.c_str());
...
void cfd::Parameter::deduce_known_info() {
  ...
  if (get_string("canonical_problem") == "constVolumeReactor") {
    update_parameter("n_var", 5);
    update_parameter("n_turb", 0);
    update_parameter("n_scalar", 0);
    return;
  }
  ...
  if (int_parameters["myid"] == 0)
    gxl::print_centered("Inviscid scheme Information", 80, '*');
  ...
  if (hybrid_inviscid_scheme != "NO") {
    ...
    if (inviscid_scheme == 2) {
      ...
      if (int_parameters["myid"] == 0)
        printf("\t->-> %-20s : reconstruction method.\n", reconstruction_name.c_str());
  ...
  if (int_parameters["myid"] == 0) {
    printf("\t->-> %-20s : inviscid scheme.\n", inviscid_scheme_name.c_str());
    printf("\t->-> %-20d : number of ghost layers.\n", ngg);
...
void cfd::Parameter::setup_default_settings() {
  ...
  int_parameters["filter_order"] = 8;
  real_parameters["filter_strength"] = 0.1;
  int_parameters["filter_frequency"] = 1;
  ...
  int_parameters["chem_advance_method"] = 0;
  int_parameters["build_arnoldi"] = 0;
  int_parameters["adaptive_time_step_rok4e"] = 1;
  real_parameters["RTol_rok4e"] = 1e-6;
  real_parameters["ATol_rok4e"] = 1e-12;
...
void cfd::Parameter::diagnose_parallel_info() {
  if (int_parameters["myid"] == 0) {
    gxl::print_centered("Parallel Information", 80, '*');
    const bool parallel = bool_parameters["parallel"];
    const int n_proc = int_parameters["n_proc"];
    if (parallel) {
      if (n_proc == 1) {
        printf("You chose parallel computation, but the number of processes is equal to 1.\n");
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      printf("\t->-> Parallel computation chosen! Number of processes: ->-> %d. \n", n_proc);
    } else {
      if (n_proc > 1) {
        printf("You chose serial computation, but the number of processes is not equal to 1, n_proc=%d.\n",
               n_proc);
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      printf("\t->-> Serial computation chosen!\n");
    }
  }
...
void cfd::Parameter::deduce_sim_info(const Species &spec) {
  ...
  if (myid == 0) {
    gxl::print_centered("Simulation Details", 80, '*');
    ...
    if (get_bool("steady")) {
      // steady
      printf("\n\t->-> %-20s : flow simulation will be conducted.\n", "Steady");
      printf("\t\t->-> %-20f : CFL number\n", get_real("cfl"));
      if (int_parameters["implicit_method"] == 0) {
        // explicit
        printf("\t\t->-> %-20s : time integration\n", "explicit");
      } else {
        // implicit
        printf("\t\t->-> %-20s : time integration\n", "implicit DPLUR");
        printf("\t\t->-> %-20d : inner iterations for DPLUR\n", get_int("DPLUR_inner_step"));
      }
      printf("\t\t->-> %-20e : convergence criteria\n", get_real("convergence_criteria"));
    } else {
      // transient simulation
      printf("\t->-> %-20s : flow simulation will be conducted.\n", "Transient");
      if (int_parameters["temporal_scheme"] == 2) {
        // dual-time stepping
        printf("\t\t->-> %-20s : temporal scheme\n", "Dual-time stepping");
        printf("\t\t->-> %-20d : inner iterations for dual-time stepping\n", get_int("inner_iteration"));
        printf("\t\t->-> %-20d : max inner iterations for dual-time stepping\n", get_int("max_inner_iteration"));
        printf("\t\t->-> %-20d : iteration adjust step\n", get_int("iteration_adjust_step"));
      } else if (int_parameters["temporal_scheme"] == 3) {
        // RK-3
        printf("\t\t->-> %-25s : temporal scheme\n", "3rd-order Runge-Kutta");
        if (bool_parameters["fixed_time_step"]) {
          printf("\t\t->-> %-25e : physical time step(s)\n", get_real("dt"));
        } else {
          printf("\t\t->-> %-25f : CFL number\n", get_real("cfl"));
        }
      } else if (int_parameters["temporal_scheme"] == 4) {
        // Wu's splitting method
        printf("\t\t->-> %-25s : temporal scheme\n", "Wu's splitting");
        if (bool_parameters["fixed_time_step"]) {
          printf("\t\t->-> %-25e : physical time step(s)\n", get_real("dt"));
        } else {
    ...
    if (get_bool("turbulence")) {
      ...
      if (need_ras_model) {
        ...
        if (int_parameters["turb_implicit"]) {
          printf("\t\t->-> %-20s : turbulence source term treatment\n", "implicit");
        }
        ...
        if (cc_flag)
          printf("\t\t->-> %-20s : compressibility correction\n", cc_method.c_str());
      }
    }
