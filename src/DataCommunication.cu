...
template<MixtureModel mix_model> void data_communication(const Mesh &mesh, std::vector<Field> &field,
  ...
  for (auto blk = 0; blk < mesh.n_block; ++blk) {
    ...
    for (auto l = 0; l < n_innFace; ++l) {
      ...
      inner_communication<mix_model><<<BPG, TPB>>>(v, nv, l, param);
    }
  }

  // Parallel communication via MPI
  if (parameter.get_bool("parallel")) {
    parallel_communication<mix_model>(mesh, field, step, parameter, param);
  }
}
...
template<MixtureModel mix_model> __global__ void inner_communication(DZone *zone, DZone *tar_zone, int i_face,
  ...
  if (idx[f.face] == face_dir) {
    ...
    // Check these two faces are wall
    const real u_this = norm3d(zone->bv(idx[0], idx[1], idx[2], 1),
                               zone->bv(idx[0], idx[1], idx[2], 2),
                               zone->bv(idx[0], idx[1], idx[2], 3));
    if (abs(u_this) > 1e-15) {
      const real u_that = norm3d(tar_zone->bv(idx_tar[0], idx_tar[1], idx_tar[2], 1),
                                 tar_zone->bv(idx_tar[0], idx_tar[1], idx_tar[2], 2),
                                 tar_zone->bv(idx_tar[0], idx_tar[1], idx_tar[2], 3));
      if (abs(u_that) < 1e-15) {
        for (int l = 0; l < 6; ++l) {
        ...
        for (int l = 0; l < param->n_scalar; ++l) {
          const real ave{
            0.5 * (tar_zone->sv(idx_tar[0], idx_tar[1], idx_tar[2], l) + zone->sv(idx[0], idx[1], idx[2], l))
          };
...
template<MixtureModel mix_model> __global__ void assign_data_received(DZone *zone, int i_face, const real *data,
  ...
  const real u_this = norm3d(bv(idx[0], idx[1], idx[2], 1),
                             bv(idx[0], idx[1], idx[2], 2),
                             bv(idx[0], idx[1], idx[2], 3));
  if (abs(u_this) > 1e-15) {
    // Wall face, do not assign the value from the other face
    if (norm3d(data[bias + 1], data[bias + 2], data[bias + 3]) < 1e-15) {
      // The other face is wall, assign the value to this face
      #pragma unroll
      for (int l = 0; l < 6; ++l) {
  ...
  if (task == 1) {
    ...
    if (idx[f.face] == face_dir) {
      for (int l = 0; l < param->n_var; ++l) {
        real ave_v = 0.5 * (zone->fFlux(idx[0], idx[1], idx[2], l) + tar_zone->fFlux(
                              idx_tar[0], idx_tar[1], idx_tar[2], l));
...
void parallel_exchange(const Mesh &mesh, std::vector<Field> &field, const Parameter &parameter, DParameter *param,
  ...
  switch (task) {
    case 1:
      n_trans = parameter.get_int("n_var") * 3; // The three viscous fluxes
      break;
    case 2:
      n_trans = 1;
      break;
...
template void data_communication<MixtureModel::Air>(const Mesh &mesh, std::vector<Field> &field,
  const Parameter &parameter, int step, DParameter *param);

template void data_communication<MixtureModel::Mixture>(const Mesh &mesh, std::vector<Field> &field,
  const Parameter &parameter, int step, DParameter *param);
}
