...
template<MixtureModel mix_model> void initialize_basic_variables(Parameter &parameter, const Mesh &mesh,
  ...
  // For some canonical problems, we have specific initialization methods
  const auto canonical_problem = parameter.get_string("canonical_problem");
  if (canonical_problem == "sinWaveProp") {
    initialize_sinWaveProp(mesh, field);
    return;
  }
...
void initialize_sinWaveProp(const Mesh &mesh, std::vector<Field> &field) {
  ...
  for (int blk = 0; blk < n_block; ++blk) {
    ...
    auto &bv = field[blk].bv;
    // auto &sv = field[blk].sv;
    const auto &x = mesh[blk].x;
    for (int k = 0; k < mz; ++k) {
      for (int j = -ngg; j < my + ngg; ++j) {
        for (int i = 0; i < mx; ++i) {
          const real rho = 1.5 + sin(2 * pi * x(i, j, k));
          bv(i, j, k, 0) = rho;
          bv(i, j, k, 1) = 1.0;
          bv(i, j, k, 2) = 0.0;
          bv(i, j, k, 3) = 0.0;
          bv(i, j, k, 4) = p;
          bv(i, j, k, 5) = p / (rho * R_air);
          // Use air as multispecies
          // bv(i, j, k, 4) = p;
          // real R = 0.232 * R_u / 31.998 + 0.768 * R_u / 28.014;
          // bv(i, j, k, 5) = p / (rho * R);
          // sv(i, j, k, 0) = 0.232;
          // sv(i, j, k, 1) = 0.768;
        }
        // Set ghost cells in streamwise direction as periodic boundary
        for (int i = -ngg; i < 0; ++i) {
          for (int l = 0; l < 6; ++l) {
            bv(i, j, k, l) = bv(mx - 1 + i, j, k, l);
          }
          // sv(i, j, k, 0) = 0.232;
          // sv(i, j, k, 1) = 0.768;
        }
        for (int i = mx; i < mx + ngg; ++i) {
          for (int l = 0; l < 6; ++l) {
            bv(i, j, k, l) = bv(i - mx + 1, j, k, l);
          }
          // sv(i, j, k, 0) = 0.232;
          // sv(i, j, k, 1) = 0.768;
        }
    ...
    cudaMemcpy(field[blk].h_ptr->bv.data(), field[blk].bv.data(), sizeof(real) * field[blk].h_ptr->bv.size() * 6,
               cudaMemcpyHostToDevice);
    // cudaMemcpy(field[blk].h_ptr->sv.data(), field[blk].sv.data(), sizeof(real) * field[blk].h_ptr->sv.size() *
...
template void initialize_basic_variables<MixtureModel::Air>(Parameter &parameter, const Mesh &mesh,
  std::vector<Field> &field, Species &species);

template void initialize_basic_variables<MixtureModel::Mixture>(Parameter &parameter, const Mesh &mesh,
  std::vector<Field> &field, Species &species);
}
