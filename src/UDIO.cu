...
// Next, the variables that would not change during the whole simulation, such as, wall distance.
MPI_Offset
cfd::write_dynamic_max_min_first_step(MPI_Offset offset, const cfd::Field &field, int ngg, const MPI_File &fp) {
...
MPI_Offset cfd::write_dynamic_max_min(MPI_Offset offset, const cfd::Field &field, int ngg, const MPI_File &fp) {
  MPI_Status status;
...
MPI_Offset cfd::write_static_array(MPI_Offset offset, const Field &field, MPI_File &fp, MPI_Datatype ty,
...
MPI_Offset cfd::write_dynamic_array(MPI_Offset offset, const Field &field, const MPI_File &fp, MPI_Datatype ty,
  long long int mem_sz) {
  MPI_Status status;
  for (int l = 0; l < UserDefineIO::n_dynamic_auxiliary; ++l) {
