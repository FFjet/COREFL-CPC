...
struct Inflow {
  ...
  // When white noise, we use y<4*delta_omega to add the moise, use a big value when not mixing layers to make the whole face available.
  real delta_omega = 1000;
...
struct Wall {
  ...
  void copy_to_gpu(Wall *d_wall, const Parameter &parameter);
  ...
  // catalytic
  int catalytic_type = 0; // 0 - non-catalytic, 1 - fully catalytic
  real *yk = nullptr;     // if catalytic, we need to specify the mass fractions at the wall.
  ...
  real blow_shock_wave_coefficient = 0.1;
  int blow_shock_wave_start_step = 0;
  int blow_shock_wave_end_step = 50;
};

struct Outflow {
  explicit Outflow(const std::string &inflow_name, Parameter &parameter);

  int label = 6;

  int if_backPressure = 0; // 0 - no back pressure, 1 - back pressure
  real backPressure = -1;
