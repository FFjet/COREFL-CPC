...
spongeZPlusEnd{parameter.get_real("spongeZPlusEnd")}*/ {
  ...
  if (nasa_7_or_9 == 7) {
    ...
    cudaMemcpy(temperature_cuts.data(), spec.temperature_range.data(), temperature_cuts.size() * sizeof(real),
               cudaMemcpyHostToDevice);
    therm_poly_coeff.allocate_memory(9, n_ranges, n_spec, 0);
    cudaMemcpy(therm_poly_coeff.data(), spec.therm_poly_coeff.data(), sizeof(real) * therm_poly_coeff.size(),
               cudaMemcpyHostToDevice);
  }
  ...
  // reaction info
  if (n_reac > 0) {
    chem_advance_method = parameter.get_int("chem_advance_method");
    build_arnoldi = parameter.get_int("build_arnoldi");
    adaptive_time_step_rok4e = parameter.get_int("adaptive_time_step_rok4e");
    RTol_rok4e = parameter.get_real("RTol_rok4e");
    ATol_rok4e = parameter.get_real("ATol_rok4e");
