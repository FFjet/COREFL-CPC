...
template<MixtureModel mix_model, OutputTimeChoice output_time_choice> BoundaryIO<
  mix_model, output_time_choice>::BoundaryIO(const Parameter &parameter_, const Mesh &mesh_, const Species &species_,
  std::vector<Field> &_field) :
  parameter(parameter_), mesh(mesh_), species(species_), field(_field),
  labels_to_output(parameter.get_int_array("output_bc")) {}

template<MixtureModel mix_model, OutputTimeChoice output_time_choice> void BoundaryIO<mix_model, output_time_choice>::
initialize() {
...
template<MixtureModel mix_model, int BoundaryType> int32_t acquire_boundary_variable_names(
  std::vector<std::string> &var_name, const Parameter &parameter, const Species &species) {
...
template<MixtureModel mix_model, OutputTimeChoice output_time_choice> void BoundaryIO<
  mix_model, output_time_choice>::write_header(const std::vector<Field> &_field) {
  const std::filesystem::path out_dir("output");
...
template<MixtureModel mix_model, OutputTimeChoice output_time_choice> void
BoundaryIO<mix_model, output_time_choice>::print_boundary() {
  const std::filesystem::path out_dir("output");
  for (int l = 0; l < labels_to_output.size(); ++l) {
    std::string file_name{out_dir.string() + "/" + name_of_boundary[l] + ".plt"};
    MPI_File fp;
