...
__device__ void cfd::compute_transport_property(int i, int j, int k, real temperature, real mw_total, real *cp,
  ...
  for (int l = 0; l < n_spec; ++l) {
    ...
    // param->vis_coeff[l] * temp / (1.0413 * pow(t_dl, -0.1193) + pow(t_dl + 0.43628, -1.6041));
  }
  ...
  for (int m = 0; m < n_spec; ++m) {
    ...
    if (const auto geom = param->geometry[m]; geom == 1 || geom == 2) {
      const real t_red{temperature * param->kb_over_eps_jk(m, m)};
      // compute ZRot
      const real tRedInv = 1 / t_red;
      temp = sqrt(tRedInv * pi) * pi;
      const real FT = 1 + 0.5 * temp + (2 + 0.25 * pi * pi) * tRedInv + temp * tRedInv;
      const real rhoD =
      ...
      if (geom == 1) {
        ...
        const real ADivPiB =
            (2.5 * vis_denom[m] - rhoD) /
            ((pi * param->ZRotF298[m] / FT * vis_denom[m] + 5 * vis_denom[m] + 2.0 * rhoD));
  ...
  temp = 1.0 / (temperature * R_u);
  for (int l = 0; l < n_spec; ++l) {
    zone->rho_D(i, j, k, l) = cp[l] * temp / vis_denom[l];
  }
}
