...
cfd::Species::Species(Parameter &parameter) {
  ...
  if (parameter.get_int("species")) {
    ...
    if (num_spec > MAX_SPEC_NUMBER) {
      printf(
        "The number of species in this simulation is %d, larger than the allowed species number %d. Please modify the CMakeLists.txt to increase the MAX_SPEC_NUMBER.\n",
    ...
    nasa_7_or_9 = parameter.get_int("therm_nasa_7_9");
    if (nasa_7_or_9 == 9) {
      has_trans = read_therm_nasa9(file, has_therm);
    } else {
      has_trans = read_therm(file, has_therm);
    }
    ...
    if (parameter.get_int("myid") == 0) {
      printf("\n");
      gxl::print_centered("Species Information", 80, '*');
      printf("\t->-> %-20d : number of species\n\t", n_spec);
      int counter_spec{0};
      for (auto &[name, label]: spec_list) {
        printf("%s\t", name.c_str());
        ++counter_spec;
...
void cfd::Species::compute_cp(real temp, real *cp) const & {
  const real t2{temp * temp}, t3{t2 * temp}, t4{t3 * temp}, iT2{1.0 / t2}, iT{1.0 / temp};
  if (nasa_7_or_9 == 7) {
    ...
    for (int i = 0; i < n_spec; ++i) {
      ...
      if (tt < temperature_range(i, 0)) {
        tt = temperature_range(i, 0);
        const real tt2{tt * tt}, tt3{tt2 * tt}, tt4{tt3 * tt}, iTT2{1.0 / tt2}, iTT{1.0 / tt};
        cp[i] = c(0, 0, i) * iTT2 + c(1, 0, i) * iTT + c(2, 0, i)
                + c(3, 0, i) * tt + c(4, 0, i) * tt2 + c(5, 0, i) * tt3 + c(6, 0, i) * tt4;
      } else if (tt > temperature_range(i, n_temperature_range[i])) {
        tt = temperature_range(i, n_temperature_range[i]);
        auto j = n_temperature_range[i] - 1;
        const real tt2{tt * tt}, tt3{tt2 * tt}, tt4{tt3 * tt}, iTT2{1.0 / tt2}, iTT{1.0 / tt};
        cp[i] = c(0, j, i) * iTT2 + c(1, j, i) * iTT + c(2, j, i)
                + c(3, j, i) * tt + c(4, j, i) * tt2 + c(5, j, i) * tt3 + c(6, j, i) * tt4;
      } else {
        for (int j = 0; j < n_temperature_range[i]; ++j) {
          if (temperature_range(i, j) <= tt && tt <= temperature_range(i, j + 1)) {
            cp[i] = c(0, j, i) * iT2 + c(1, j, i) * iT + c(2, j, i)
                    + c(3, j, i) * tt + c(4, j, i) * t2 + c(5, j, i) * t3 + c(6, j, i) * t4;
            break;
          }
        }
      }
...
bool cfd::Species::read_therm(std::ifstream &therm_dat, bool read_from_comb_mech) {
  ...
  if (!read_from_comb_mech) {
    read_until(therm_dat, input, "THERMO", gxl::Case::upper); // "THERMO"
  }
  #ifdef HighTempMultiPart
  ...
  while (getline_to_stream(therm_dat, input, line, gxl::Case::upper)) {
    ...
    if (key == "END") {
      if (n_read < n_spec) {
        printf("The thermodynamic data aren't enough. We need %d species info but only %d are supplied.\n", n_spec,
               n_read);
      }
      continue;
    }
    if (key == "REACTIONS" || input == "REAC") break;
    if (key == "TRANSPORT" || key == "TRAN" || key == "TRANS") {
      has_trans = true;
      break;
    }
    ...
    if (read) {
      ...
      gxl::getline(therm_dat, input);
      gxl::getline(therm_dat, input);
      getline_to_stream(therm_dat, input, line, gxl::Case::upper);
      line >> key;
      #endif
      continue;
    }
...
bool cfd::Species::read_therm_nasa9(std::ifstream &therm_dat, bool read_from_comb_mech) {
  std::string input{};
  if (!read_from_comb_mech) {
    read_until(therm_dat, input, "THERMO", gxl::Case::upper); // "THERMO"
  }
  ...
  std::istringstream line(input);
  bool has_trans{false};
  std::vector<std::vector<real>> temporary_range(n_spec, {T_low, T_mid, T_high, T_hyper});
  std::vector<int> n_temperature_spec(n_spec, 4);
  std::vector<gxl::MatrixDyn<real>> therm_poly_tempo(n_spec);
  int range_max = 2;
  while (getline_to_stream(therm_dat, input, line, gxl::Case::upper)) {
    if (input.empty() || input[0] == '!') {
    ...
    range_max = std::max(range_max, n_temperature_range[curr_sp]);
    temporary_range[curr_sp].resize(n_part + 1);
    therm_poly_tempo[curr_sp].resize(n_temperature_range[curr_sp], 9);

    std::string comp_str{};
    for (int i = 0; i < 5; ++i) {
      comp_str.assign(input, 10 + i * 8, 8);
      gxl::trim_left(comp_str);
      if (comp_str.empty() || comp_str.starts_with('0')) break;
      gxl::to_stringstream(comp_str, line);
      line >> key;
    ...
    mw[curr_sp] = std::stod(key.assign(input, 52, 13));

    // Read the thermodynamic fitting coefficients
    for (int p = 0; p < n_part; ++p) {
      std::getline(therm_dat, input);
      key.assign(input, 0, 11);
      temporary_range[curr_sp][p] = std::stod(key);
      key.assign(input, 11, 11);
      temporary_range[curr_sp][p + 1] = std::stod(key);
      // Read polynomial coefficients for the n_range ranges
      std::getline(therm_dat, input);
      gxl::replace(input, 'D', 'E');
      std::string cs1{}, cs2{}, cs3{}, cs4{}, cs5{};
      cs1.assign(input, 0, 16);
      cs2.assign(input, 16, 16);
      cs3.assign(input, 32, 16);
      cs4.assign(input, 48, 16);
      cs5.assign(input, 64, 16);
      therm_poly_tempo[curr_sp](p, 0) = std::stod(cs1);
      therm_poly_tempo[curr_sp](p, 1) = std::stod(cs2);
      therm_poly_tempo[curr_sp](p, 2) = std::stod(cs3);
      therm_poly_tempo[curr_sp](p, 3) = std::stod(cs4);
      therm_poly_tempo[curr_sp](p, 4) = std::stod(cs5);
      std::getline(therm_dat, input);
      gxl::replace(input, 'D', 'E');
      ...
      cs4.assign(input, 48, 16);
      cs5.assign(input, 64, 16);
      therm_poly_tempo[curr_sp](p, 5) = std::stod(cs1);
      therm_poly_tempo[curr_sp](p, 6) = std::stod(cs2);
      therm_poly_tempo[curr_sp](p, 7) = std::stod(cs4);
      therm_poly_tempo[curr_sp](p, 8) = std::stod(cs5);
    }
  ...
  // After reading all info, we need to resize the arrays of thermodynamic info
  temperature_range.resize(n_spec, range_max + 1);
  therm_poly_coeff.resize(9, range_max, n_spec, 0); // This is column major
  for (int l = 0; l < n_spec; ++l) {
    for (int i = 0; i < n_temperature_spec[l]; ++i) {
      temperature_range(l, i) = temporary_range[l][i];
    }
...
void cfd::Species::read_tran(std::ifstream &tran_dat) {
  ...
  while (getline_to_stream(tran_dat, input, line, gxl::Case::upper)) {
    ...
    if (key.starts_with("END") || key.starts_with("REACTIONS") || key.starts_with("REAC")) {
      if (n_read < n_spec) {
        printf("The transport data aren't enough. We need %d species info but only %d are supplied.\n", n_spec,
               n_read);
      }
      break;
    }
  ...
  for (int j = 0; j < n_spec; ++j) {
    ...
    kb_over_eps_jk(j, j) = 1.0 / eps_kb[j];
...
cfd::Reaction::Reaction(Parameter &parameter, const Species &species) {
  ...
  read_until(file, input, reac_candidate, gxl::Case::keep);
  std::istringstream line(input);
  std::string key{};
  line >> key >> key;
  if (!key.empty()) {
  ...
  getline_to_stream(file, input, line, gxl::Case::upper);
  while (input != "END") {
  ...
  if (parameter.get_int("myid") == 0) {
    ...
    printf("\t\t->-> %-25s : chemical source treatment\n", method.c_str());
  }
...
void cfd::Reaction::read_reaction_line(std::string input, int idx, const Species &species) {
  ...
  std::string token;
  while (line >> token) {
    if (token == "!" || token == "//" || token[0] == '!' || (token.size() >= 2 && token[0] == '/' && token[1] == '/')) {
      break; //The rest are comments
    }
    tokens.push_back(token);
  }
  // The last three tokens are A, b, Ea
  if (tokens.size() < 4) {
    printf("Error: reaction line '%s' is not complete.\n", input.c_str());
    exit(1);
  }
  input.clear();
  ...
  // Now input only contains the reaction equation.
...
std::string cfd::Reaction::get_auxi_info(std::ifstream &file, int idx, const Species &species, bool &is_dup) {
  std::string input, key;
  while (gxl::getline(file, input, gxl::Case::upper)) { //&&input.find("=")==std::string::npos
    ...
    if (key == "REV") {
      ...
      Ea2[idx] /= R_c;
      continue;
    } else if (key == "TROE") { //Troe form needs the following coefficients
      label[idx] = 6;
