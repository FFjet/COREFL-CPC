...
\(\LL_{i+1/2}\)、\(\RR_{i+1/2}\)。有限差分通量在计算坐标中离散，
因此实际进入 WENO 的是带 Jacobian 的逆变通量
\[
\widetilde{\FF}_m=J_m\FF_m .
\]
谱半径也必须使用同一计算坐标缩放：
\[
\widehat{\lambda}_{\max}
=\max_m\left[J_m\left(|U_k|+c|\kk|\right)\right]_m .
\]
有限差分 WENO 使用局部 Lax--Friedrichs 分裂。对第 \(l\) 个特征行，
正确的分裂是
\[
\widehat{f}^{\pm}_{l,m}
=\frac{1}{2}\left[
J_m\,\LL_{l,i+1/2}\FF_m
\pm \widehat{\lambda}_{\max}\,\LL_{l,i+1/2}\QQ_m
\right].
\]
也就是说，Jacobian 只乘物理通量投影 \(L\FF\)，
耗散项使用已经带 Jacobian 的全 stencil 最大谱半径
\(\widehat{\lambda}_{\max}\)。不能写成
\(\frac12 J_m(L\FF\pm\lambda_{\max}L\QQ)\)，否则耗散项的
Jacobian 缩放与局部点 \(m\) 错配。
分别做 WENO5 或 WENO7 重构：
\[
\hat{f}_{l,i+1/2}
=\mathcal{WENO}(\widehat{f}^{+}_{l,i-r},\ldots,\widehat{f}^{+}_{l,i+s})
+\mathcal{WENO}(\widehat{f}^{-}_{l,i+1-r},\ldots,\widehat{f}^{-}_{l,i+1+s}).
\]
最后回投影：
\[
...
stencil 最大谱半径在源码中对应
\[
\code{max\_lambda}=\max_m
\code{F[6][m]}\left(\left|\code{F[0][m]}\right|+\code{F[5][m]}\right).
\]
每个 stencil 点先计算
\(\code{LFm}=\LL_{i+1/2}\FF_m\)，再执行
\[
\code{LFm *= F[6][m]},
\qquad
\code{vPlus/Minus}=\frac12(\code{LFm}\pm
\code{max\_lambda}\,\code{LQm}).
\]
这里 \(\code{LQm}=\LL_{i+1/2}\QQ_m\)。该写法与上面的
\(\widehat{\lambda}_{\max}\) 分裂一致。
...
8 阶 collocated 与 2 阶 face 通量均在总能量 \texttt{f/g/hFlux(...,4)} 中加入
\(\kappa_{\ve}\nabla T_{\ve}\)，并在
...
源码中 8 阶 collocated 粘性通量和 2 阶 face 粘性通量均写入
\texttt{fFlux/gFlux/hFlux}，并按守恒变量索引对齐；因此总能量为
\texttt{(...,4)}，第 \(l\) 个物种为 \texttt{(...,5+l)}，
\(\rho E_{\ve}\) 为 \texttt{(...,5+i\_eve)}，被动标量使用其守恒变量索引
\texttt{lc}。
对应代码为
\[
\begin{array}{c|c|c}
